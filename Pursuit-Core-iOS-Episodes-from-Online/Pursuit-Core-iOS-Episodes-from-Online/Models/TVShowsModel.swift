...
struct TVShowsWrapper: Codable{
    let show: Shows
    
    static func getShow(userInput: String?,completionHandler: @escaping (Result<[TVShowsWrapper],AppError>) -> () ) {
      ...
      print(url)
            ...
            case .success(let data):
                do {
                    let decodedShow = try JSONDecoder().decode([TVShowsWrapper].self, from: data)
